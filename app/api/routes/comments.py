...
def show(comment_id: int, viewer: Viewer, service: Comments) -> CommentResponse:
    return CommentResponse.model_validate(service.get(comment_id, viewer))


@router.put("/{comment_id}")
def update(
    comment_id: int, data: CommentWrite, user: CurrentUser, service: Comments
) -> CommentResponse:
    return CommentResponse.model_validate(service.update(user, comment_id, data))


@router.delete("/{comment_id}", status_code=status.HTTP_204_NO_CONTENT)
def destroy(comment_id: int, user: CurrentUser, service: Comments) -> None:
