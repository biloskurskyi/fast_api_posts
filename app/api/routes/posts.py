...
@router.get("/{post_id}")
def show(post_id: ResourceId, service: Posts) -> PostResponse:
    return PostResponse.model_validate(service.get(post_id))


@router.put("/{post_id}")
def update(post_id: ResourceId, data: PostWrite, user: CurrentUser, service: Posts) -> PostResponse:
    return PostResponse.model_validate(service.update(user, post_id, data))


@router.delete("/{post_id}", status_code=status.HTTP_204_NO_CONTENT)
def destroy(post_id: ResourceId, user: CurrentUser, service: Posts) -> None:
